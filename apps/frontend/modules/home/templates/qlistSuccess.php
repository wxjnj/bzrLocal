	...
	<div class="expert_cont clearfix">
		...
		<div class="r">
			...
			<div class="expert_answer">
				<h4><a href="javascript:void(0)"><?php echo $type=='finished'?'已解答问题':'未解答问题'?></a></h4>
				<ul>
				<?php foreach($pager as $question):?>
					<li>
					<?php if($type=='finish'):?>
					<a href="<?php echo url_for('@qshow?token='.$question->getToken());?>">
					<?php else :?>
					<a href="#">
					<?php endif;?>
					<span class="green">[<?php echo $question->getUname();?>]</span><?php echo Tools::cur_str_utf8($question->getTitle(), 30,0,2)?><span class="person"><?php echo $question->getEname();?></span>
					</a>
					</li>
				<?php endforeach;?>
				</ul>
				...
				<div class="page_wrap">
					<div class="page">
						<a href="<?php echo url_for('@qlist?page='.$pager->getPreviousPage().'&type='.$type);?>">上一页</a>
						<?php for($i = 1;$i<=$pager->getLastPage();$i++):?>
							<?php if($i == $pager->getPage()):?>
				    			<a href="javascript:viod(0)" class="on"><?php echo $i;?></a>
				    		<?php else:?>
				    			<a href="<?php echo url_for('@qlist?page='.$i.'&type='.$type);?>"><?php echo $i;?></a>
				    		<?php endif;?>
				    	<?php endfor;?>
						<a href="<?php echo url_for('@qlist?page='.$pager->getNextPage().'&type='.$type);?>">下一页</a>
					</div>
				</div>
				<?php endif;?>
			</div>
