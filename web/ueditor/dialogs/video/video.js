...
(function(){
    ...
    videoUpload.init = function(){
        switchTab("videoTab");
        createAlignButton( ["videoFloat_upload"] );
        addUrlChangeListener($G("videoUrl_upload"));
        addOkListener1();
        addSearchListener();
        ...
        (function(){
            var img = editor.selection.getRange().getClosedNode(),url;
            if(img && img.className == "edui-faked-video"){
                $G("videoUrl_upload").value = url = img.getAttribute("_url");
                $G("videoWidth_upload").value = img.width;
                $G("videoHeight_upload").value = img.height;
                updateAlignButton1(img.getAttribute("align"));
            }
            createPreviewVideo1(url);
        })();
    };
    /**
        ...
        dialog.onok = function(){
            ...
            switch(currentTab){
                ...
                case "videoSearch":
                    ...
                    break;
                case "videoLocalSearch":
                    return insertSearch("videoLocalSearchList");
                    break;
            }
        };
    ...
    function addOkListener1(){
        dialog.onok = function(){
            $G("preview_upload").innerHTML = "";
            var currentTab =  findFocus("tabHeads","tabSrc");
            switch(currentTab){
                case "video":
                    return insertSingle();
    ...
    function updateAlignButton1( align ) {
        var aligns = $G( "videoFloat_upload" ).children;
        for ( var i = 0, ci; ci = aligns[i++]; ) {
            if ( ci.getAttribute( "name" ) == align ) {
                if ( ci.className !="focus" ) {
                    ci.className = "focus";
    ...
    function insertSingle1(){
        var width = $G("videoWidth_upload"),
            height = $G("videoHeight_upload"),
            url=$G('videoUrl_upload').value,
            align = findFocus("videoFloat_upload","name");
        if(!url) return false;
        if ( !checkNum( [width, height] ) ) return false;
        editor.execCommand('insertvideo1', {
            url: convert_url(url),
            width: width.value,
            height: height.value,
            align: align
    ...
    function createPreviewVideo1(url){
        if ( !url )return;
        if(!endWith(url,[".swf",".flv",".wmv"])){
            $G("preview_upload").innerHTML = "您输入的视频地址有误，请检查后确认！";
            return;
        }
