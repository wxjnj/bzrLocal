        ...
        applyInlineStyle : function( tagName, attrs ,list) {
            ...
            while ( current && (domUtils.getPosition( current, end ) & domUtils.POSITION_PRECEDING) ) {
                ...
                if ( current.nodeType == 3 || dtd[tagName][current.tagName] ) {
                    ...
                    if(attrs && attrs.islink == '1')
                    {//zhaoyun 2013-07-11
                    	frag.textContent = attrs.title;
            ...
            if(opt.length > 0){
                ...
                if(opt.length == 1){
                    str = '<img src="'+ci.src+'" '+ (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src +'" ':'') +
                            ...
                            (ci.title?' title="'+ci.title+'"':' title=""') + ' border="'+ (ci.border||0) + '" hspace = "'+(ci.hspace||0)+'" vspace = "'+(ci.vspace||0)+'" />';
                    if(ci.title){
                    	str +='<br />'+ci.title;
                    }
...
(function() {
    ...
    function doLink(range,opt){
        ...
        if ( range.collapsed ) {
        	...
        	//alert(range.endContainer.textContent);
        	//alert(range.startContainer.textContent);
        	//alert(range.setEndAfter( pre ).extractContents());
        	//range.setEndAfter( pre ).extractContents().textContent = '';
...
UE.plugins['video'] = function (){
    ...
    function creatInsertStr1(url,width,height,align,toEmbed,addParagraph){
//        return  '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"' +
//        'codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0"' +
//        'width="' + width  + '"' +
//        'height="' + height  + '">' +
//        '<param name="movie" value="/flash/videoplayer3.swf?videoURL=' + url +'" />'+
//        '<param name="quality" value="high" />'+
//        '<embed src="/flash/videoplayer3.swf?videoURL='+ url + '" quality="high"'+
//        'pluginspage="http://www.macromedia.com/go/getflashplayer"'+
//        'type="application/x-shockwave-flash" '+
//        'width="'+ width +'" height="'+height+'"></embe1d></object>';
    	width = 520;
    	height = 390;
        return  '<embed type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
                ' src="/flash/videoplayer3.swf?videoURL=' + url + '&btncolor=0x333333&accentcolor=0x31b8e9&txtcolor=0xdddddd&volume=30&autoload=on&autoplay=off&vTitle=Super Mario Brothers Lego Edition&showTitle=yes" width="' + width  + '" height="' + height  + '" align="' + align + '"' +
                ( align !="none" ? ' style= "'+ ( align == "center"? "display:block;":" float: "+ align )  + '"' :'' ) +
                ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
    }
    function creatInsertStr2(url,width,height,align,toEmbed,addParagraph){
    	width = 520;
    	height = 390;
      return  '<embed type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
              ' src="' + url + '&btncolor=0x333333&accentcolor=0x31b8e9&txtcolor=0xdddddd&volume=30&autoload=on&autoplay=off&vTitle=Super Mario Brothers Lego Edition&showTitle=yes" width="' + width  + '" height="' + height  + '" align="' + align + '"' +
              ( align !="none" ? ' style= "'+ ( align == "center"? "display:block;":" float: "+ align )  + '"' :'' ) +
              ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" ><br/>';
  }
    ...
    me.commands["insertvideo1"] = {
        execCommand: function (cmd, videoObjs){
            ...
            for(var i=0,vi,len = videoObjs.length;i<len;i++){
                 vi = videoObjs[i];
                 html.push(creatInsertStr1( vi.url, vi.width || 420,  vi.height || 280, vi.align||"none",false,false));
            }
            me.execCommand("inserthtml",html.join(""));
        },
        queryCommandState : function(){
    ...
    me.commands["insertvideo"] = {
            execCommand: function (cmd, videoObjs){
                ...
                for(var i=0,vi,len = videoObjs.length;i<len;i++){
                     vi = videoObjs[i];
                     html.push(creatInsertStr2( vi.url, vi.width || 420,  vi.height || 280, vi.align||"none",false,true));
                }
                me.execCommand("inserthtml",html.join(""));
            },
            queryCommandState : function(){
