...
$targetFolder = '/uploads/attachment'; // Relative to the root
if (!empty($_FILES)) {
	$tempFile = $_FILES['Filedata']['tmp_name'];
	$targetPath = $_SERVER['DOCUMENT_ROOT'] . $targetFolder;
	
	// Validate the file type
	$fileTypes = array('flv','rp','jpg','jpeg','gif','png','txt','xls','doc','rar','zip','docx','xlsx','ppt','pptx'); // File extensions
// 	$fileTypes = array('flv','wmv','vob','avi','mp4'); // File extensions
	...
	if (in_array($extension,$fileTypes)) {
		...
		echo '{"name":"'.$filename.'","true_name":"'.$file_true_name.'","size":"'.$size.'"}';
...
// 		}
		...
		echo '{"name":"'.''.'","true_name":"'.'无效的文件，请检查文件的合法性'.'","size":"'.''.'"}';
	}
}
?>